...
('human-gate', 'Human Gate', 'component', 'supervisor-layer', 'pink', '⛳', 'Three modes: full-auto, approve-goals, approve-all. Plus write fence: dangerous ops require approval even in full-auto. Also surfaces escalation requests from the Worker. Write fence per-instance: Meta-Agent config mutations and Worker destructive ops can have independent gate policies. Gate mode is a runtime flag — switch between modes without restarting any process.', '["full-auto","approve-goals","approve-all","write fence","runtime flag"]', 23),
